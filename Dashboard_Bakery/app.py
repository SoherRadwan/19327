...
def process_environmental_data(text):
    ...
    # Add timestamp column with 1-second intervals
    base_time = datetime.now() - timedelta(minutes=len(df))
    df["timestamp"] = [base_time + timedelta(minutes=x) for x in range(len(df))]
